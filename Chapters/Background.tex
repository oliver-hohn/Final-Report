...
\end{figure}
\par On the resulting tree, the algorithm dictates the removal of determiners. The "DT" parent tag indetifies a determiner. However, not all the parent identifiers with this tag are removed, only ones that have a child labelled "the" or "a". The resulting tree after applying this rule is given by Figure \ref{tree:removeDeterminers}.
\begin{figure}[h]
\resizebox{\linewidth}{!}{
\Tree
	[.S 
